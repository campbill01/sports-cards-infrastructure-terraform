terraform {
  ...
  backend "s3" {
    ...
    key            = "manufacturing-orchestrator/us-east-1/prod/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
...
# Provider for cross-region RDS replica
provider "aws" {
  alias  = "replica"
  region = "us-west-2"
...
module "database" {
  ...
  # Cross-region replica to us-west-2 for disaster recovery
